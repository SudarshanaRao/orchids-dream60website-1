...
export function useActivityTracker(userId: string | null, username: string | null, currentPath: string) {
  ...
  const sendBeaconData = useCallback((endpoint: string, data: Record<string, unknown>) => {
    try {
      const blob = new Blob([JSON.stringify(data)], { type: 'application/json' });
      return navigator.sendBeacon(`${BACKEND_URL}/user-activity${endpoint}`, blob);
    } catch (error) {
      console.debug('Beacon error:', error);
      return false;
    }
  }, []);

  const flushPendingData = useCallback(async (useBeacon = false) => {
    ...
    if (useBeacon) {
      sendBeaconData('/batch', batchData);
    } else {
      await sendRequest('/batch', batchData);
    }

    pendingPageViews.current = [];
    pendingInteractions.current = [];
  }, [userId, sendRequest, sendBeaconData]);
  ...
  const endSession = useCallback(async (useBeacon = false) => {
    if (!userId || !sessionIdRef.current) return;

    await flushPendingData(useBeacon);
    ...
    if (useBeacon) {
      sendBeaconData('/session/end', endData);
    } else {
      await sendRequest('/session/end', endData);
    }

    sessionIdRef.current = null;
    currentPageRef.current = null;
    isInitialized.current = false;
  }, [userId, sendRequest, sendBeaconData, flushPendingData]);
  ...
  useEffect(() => {
    if (isInitialized.current && currentPath !== lastPath.current) {
      trackPageView();
    }
  }, [currentPath, trackPageView]);

  return { trackInteraction, endSession: () => endSession(false) };
}
