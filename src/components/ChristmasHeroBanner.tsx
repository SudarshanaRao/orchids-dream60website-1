...
export const ChristmasHeroBanner: React.FC<ChristmasHeroBannerProps> = ({ user, onJoinNow }) => {
  return (
    <section className="relative w-full h-screen overflow-hidden bg-[#0a1a2f]">
      ...
      <div className="absolute inset-0 pointer-events-none z-20">
        ...
        {/* Main Content Area */}
        <div className="relative z-30 h-full max-w-[1440px] mx-auto px-4 sm:px-12 md:px-20 lg:px-24 flex flex-col items-center justify-end pb-4 sm:pb-6 lg:pb-8">
          <motion.div
          ...
          >
            <button
              onClick={onJoinNow}
              className="group relative px-4 py-1.5 sm:px-5 sm:py-2 lg:px-4 lg:py-1.5 bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 rounded-full font-bold text-black uppercase tracking-widest shadow-[0_0_20px_rgba(251,191,36,0.4)] hover:shadow-[0_0_40px_rgba(251,191,36,0.6)] transform hover:-translate-y-1 transition-all duration-300 active:scale-95"
            >
              <span className="relative z-10 flex items-center gap-2 text-xs sm:text-sm lg:text-xs">
                Join Now
                <ChevronRight className="w-3 h-3 sm:w-4 sm:h-4 lg:w-3 lg:h-3 group-hover:translate-x-1 transition-transform" />
              </span>
