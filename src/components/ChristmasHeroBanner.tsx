...
export const ChristmasHeroBanner: React.FC<ChristmasHeroBannerProps> = ({ user, onJoinNow }) => {
  return (
    <section className="relative w-full h-[32vh] sm:h-[450px] md:h-[550px] lg:h-[650px] overflow-hidden bg-[#0a1a2f]">
      ...
      {/* Main Content Area */}
      <div className="relative z-30 h-full max-w-[1440px] mx-auto px-4 sm:px-12 md:px-20 lg:px-24 flex flex-col items-center justify-end pb-6 sm:justify-center sm:pb-0">
        <div className="max-w-3xl space-y-2 sm:space-y-6 text-center">
          ...
          {/* Headline & Subheadline (Headline Removed as requested) */}
          <div className="space-y-2 sm:space-y-4">
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 1, delay: 0.6 }}
              className="text-sm sm:text-lg md:text-xl text-gray-100 font-semibold max-w-lg leading-tight select-none mx-auto drop-shadow-md"
            >
          ...
          >
            <motion.button
              onClick={onJoinNow}
              whileTap={{ scale: 0.95 }}
              className="relative flex items-center justify-center gap-2 bg-[#B71C1C]/90 text-white px-6 sm:px-12 py-2 sm:py-4 rounded-full font-bold text-sm sm:text-lg shadow-2xl backdrop-blur-sm hover:bg-[#B71C1C] transition-colors"
            >
              <span className="relative z-10">{user ? 'Explore Prizes' : 'Join Now'}</span>
