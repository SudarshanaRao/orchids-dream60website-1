...
// Trying sha256 digest as key
const key = crypto.createHash('sha256').update(AIRPAY_USERNAME + "~:~" + AIRPAY_PASSWORD).digest();

function decrypt(data, secretKey) {
  try {
    const iv = data.slice(0, 16);
    const encryptedData = Buffer.from(data.slice(16), 'base64');
    const decipher = crypto.createDecipheriv('aes-256-cbc', secretKey, Buffer.from(iv, 'utf-8'));
...
console.log('Decrypted:', decrypt(responseData, key));
